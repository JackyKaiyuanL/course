...
class RotaryPositionEmbedding(nn.Module):
    ...
    def __init__(self, d_model: int, max_seq_len: int, theta: float = 10000.0):
        ...
        inv_freq = 1.0 / (theta ** (torch.arange(0, d_model, 2, dtype=torch.float32) / d_model))
    ...
    def forward(self, x: Tensor, token_positions: Tensor) -> Tensor:
        ...
        if x.dim() == 4:  # (B, H, T, D)
            if cos.dim() == 2:  # (T, D)
                ...
                sin = sin.unsqueeze(1)
        elif x.dim() == 3:  # (B, T, D) or (T, D)
            # (T, D) broadcasts fine; (B, T, D) matches fine
            pass
...
def scaled_dot_product_attention(
    ...
    scores = Q @ K.transpose(-2, -1) / math.sqrt(d_k)  # (..., q, k)

    if mask is not None:
        # mask: True = keep, False = mask out
        scores = scores.masked_fill(~mask, -1e9)

    attn = softmax(scores, dim=-1)  # (..., q, k)
    return attn @ V  # (..., q, d_v)
...
def count_flops_per_token(
    ...
    # Per-layer MACs per token:
    # Q,K,V,O projections: 4 * d_model * d_model
    macs_proj = 4 * d_model * d_model

    # Attention: (scores + weighted sum) ~ 2 * context_length * d_model
    macs_attn = 2 * context_length * d_model

    # FFN (SwiGLU): w1 + w3 + w2 = 3 * d_model * d_ff
    macs_ffn = 3 * d_model * d_ff

    macs_per_layer = macs_proj + macs_attn + macs_ffn

    # Final output projection to vocab: d_model * vocab_size
    macs_out = d_model * vocab_size

    # Convert MACs to FLOPs (~2 FLOPs per MAC)
    flops = 2 * (num_layers * macs_per_layer + macs_out)
    return int(flops)
...
def estimate_memory_bytes(
    ...
    base_params = vocab_size * d_model + d_model * vocab_size + d_model

    
    layer_params = (2 * d_model) + (4 * d_model * d_model) + (3 * d_model * d_ff)

    total_params = base_params + num_layers * layer_params
